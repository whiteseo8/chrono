...
$id=isset($_GET['id'])?(int)$_GET['id']:'';$ac=isset($_GET['ac'])?$_GET['ac']:'';if($ac=='del'){$file=VV_DATA.'/config/'.$id.'.php';if(@unlink($file))ShowMsg("恭喜你,删除成功！",'caiji_config.php',500);}if($ac=='yulan'){require(VV_INC."/caiji.class.php");require(VV_DATA.'/rules.php');exit;}if($ac=='savecollectid'){$config=array('collectid'=>$_GET['collectid']);$config=@array_merge($v_config,$config);if($config){arr2file(VV_DATA."/config.php",$config);}ShowMsg("恭喜你,修改成功！",'caiji_config.php',500);}if($ac=='status'){$collectid=(int)$_GET['collectid'];$file=VV_DATA.'/config/'.$collectid.'.php';$sid=intval($_GET['sid']);if(!is_file($file))ShowMsg("采集配置文件不存在",'-1',2000);$caiji_config=require_once($file);if($caiji_config){$caiji_config['collect_close']=$sid;arr2file($file,$caiji_config);}ShowMsg("恭喜你,修改成功！",'?',500);}if($ac=='save'){$config=$_POST['con'];foreach($config as $k=>$vo){if(is_array($config[$k])){foreach($config[$k]as $kk=>$vv){$config[$k][$kk]=utf2gbk(get_magic(trim($vv)));}}else{$config[$k]=utf2gbk(get_magic(trim($config[$k])));}}$zdy=$_POST['zdy'];if($zdy){foreach($zdy as $k=>$vo){foreach($vo as $kk=>$vv){$zdy[$k][$kk]=utf2gbk(get_magic(trim($vv)));if(in_array($kk,array('name','ename'))&&$zdy[$k][$kk]==''){unset($zdy[$k]);}}}}$config['zdy']=$zdy;if($config['replacerules']){if(!preg_match('#\{vivicut\}#',$config['replacerules'])){}}if($config['plus']){$config['plus']=implode(',',$config['plus']);}else{$config['plus']='';}if($config['siftrules']){$config['siftrules']=str_replace(array("\r\n","\r","\n"),'[cutline]',$config['siftrules']);$siftrules=explode('[cutline]',$config['siftrules']);foreach($siftrules as $k=>$vo){if(!preg_match('#\{vivi\s+replace\s*=\s*\'([^\']*)\'\s*\}(.*)\{/vivi\}$#',trim($vo))){ajaxReturn(array('status'=>0,'info'=>"过滤规则的正则表达式格式不正确"));}}$config['siftrules']=implode("[cutline]",$siftrules);}if($config['replacerules_before']){if(!preg_match('#\{vivicut\}#',$config['replacerules_before'])){}}if($config['siftrules_before']){$config['siftrules_before']=str_replace(array("\r\n","\r","\n"),'[cutline]',$config['siftrules_before']);$siftrules_before=explode('[cutline]',$config['siftrules_before']);foreach($siftrules_before as $k=>$vo){if(!preg_match('#\{vivi\s+replace\s*=\s*\'([^\']*)\'\s*\}(.*)\{/vivi\}$#',trim($vo))){ajaxReturn(array('status'=>0,'info'=>"过滤规则的正则表达式格式不正确"));}}$config['siftrules_before']=implode("[cutline]",$siftrules_before);}$file=VV_DATA.'/config/'.$id.'.php';if(is_file($file)){$caiji_config=require_once($file);$config=array_merge($caiji_config,$config);}$config=array_merge($config,array('siftags'=>@$_POST['siftags'],'time'=>time()));$result=arr2file($file,$config);if($result===false){ajaxReturn(array('status'=>1,'info'=>"修改失败，检查文件写入权限！"));}ajaxReturn(array('status'=>1,'info'=>"恭喜你,修改成功！"));}if($ac=='saveimport'){$text=trim($_POST['import_text']);$id=isset($_POST['id'])?$_POST['id']:'';if(!$id){$arr=glob(VV_DATA.'/config/*.php');if(!OoO0o0O0o()&&count($arr)>=2)ShowMsg('导入失败，未授权只能有2条规则','-1',6000);if($arr){$arr=array_map('basename',$arr);$arr=array_map('intval',$arr);$id=max($arr)+1;}}if(!$id){$id=1;}$file=VV_DATA.'/config/'.$id.'.php';if(preg_match('#^VIVI:#',$text)){if(!preg_match('#:END$#',$text)){ShowMsg('该规则不合法，采集规则为：VIVI:base64代码:END !','-1',6000);}$notess=explode(':',$text);$config=$notess[1];$config=unserialize(base64_decode(preg_replace("#[\r\n\t ]#",'',$config)))OR die('配置字符串有错误！');}else{ShowMsg("该规则不合法，无法导入！",'-1',6000);}arr2file($file,$config);ShowMsg("恭喜你,导入成功！",'caiji_config.php',2000);}echo ADMIN_HEAD; ?>
<body>
<div class="right">
 <?php include "welcome.php"; ?>
  <div class="right_main">
<?php if($ac==''){$dir=VV_DATA.'/config';$filearr=scandirs($dir);$temp=array();foreach($filearr as $file){if($file<>'.'&&$file<>'..'){if(is_file("$dir/$file")){if(!preg_match('#^\d+\.php$#',$file)){continue;}$thisid=str_replace('.php','',$file);$file=VV_DATA.'/config/'.$file;$caiji_config=require_once($file);$temp[]=array_merge($caiji_config,array('id'=>$thisid));}}}foreach($temp as $key=>$row){$volume[$key]=$row['id'];}@array_multisort($volume,SORT_DESC,$temp);if(!OoO0o0O0o())$temp=array_slice($temp,0,2); ?>
...
<?php if($temp){foreach($temp as $k=>$vo){ ?>
	<tr nowrap class="firstalt">
		<td width="60" align="center"><?php echo $vo['id']==$v_config['collectid']?'<font color="red">默认节点</font>':'<a href="?ac=savecollectid&collectid='.$vo['id'].'&sid=0" title="点击设为默认节点">设为默认</a>'; ?></td>
...
<?php }elseif($ac=='export'){$file=VV_DATA.'/config/'.$id.'.php';if(!is_file($file))ShowMsg("采集配置文件不存在",'-1',2000);$caiji_config=require_once($file);$basecon="VIVI:".base64_encode(serialize($caiji_config)).":END"; ?>
<table width="98%" border="0" cellpadding="4" cellspacing="1" class="tableoutline">
	<tbody>
		<tr nowrap class="tb_head">
			<td><h2>导出采集规则</h2></td>
	...
	<tr nowrap class="firstalt">
		<td align="center"><textarea style="height: 350px;width:95%;padding:5px;background:#eee;" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo $basecon; ?></textarea></td>
	</tr>
</table>
<?php }elseif($ac=='import'){$tinfo='';if($id){$file=VV_DATA.'/config/'.$id.'.php';if(!is_file($file))ShowMsg("采集配置文件不存在",'-1',3000);$caiji_config=require_once($file);$tinfo='( 覆盖['.$caiji_config['name'].']？)<input type="hidden" name="id" value="'.$id.'" />';} ?>
<table width="98%" border="0" cellpadding="4" cellspacing="1" class="tableoutline">
...
<?php }elseif($ac=='xiugai'||$ac=='add'){if($ac=='xiugai'){$file=VV_DATA.'/config/'.$id.'.php';if(!is_file($file))ShowMsg("采集配置文件不存在",'-1',3000);$caiji_config=require_once($file);if($caiji_config['siftrules']){$caiji_config['siftrules']=implode("\r\n",explode('[cutline]',$caiji_config['siftrules']));}if($caiji_config['siftrules_before']){$caiji_config['siftrules_before']=implode("\r\n",explode('[cutline]',$caiji_config['siftrules_before']));}if(empty($caiji_config['siftags']))$caiji_config['siftags']=array('123');$caiji_config['resdomain']=$caiji_config['resdomain']?$caiji_config['resdomain']:$caiji_config['other_imgurl'];}else{$caiji_config=array('name'=>'','replace'=>'','charset'=>'gb2312','from_url'=>'','resdomain'=>'','siftags'=>array(),'siftrules'=>'','replacerules'=>'','rewrite'=>'','licence'=>'','from_title'=>'','search_url'=>'',);$arr=glob(VV_DATA.'/config/*.php');$id=1;if($arr){$arr=array_map('basename',$arr);$arr=array_map('intval',$arr);$id=max($arr)+1;}} ?>
...
<table width="98%" border="0" cellpadding="4" cellspacing="1" class="tableoutline">
		...
		<tr nowrap class="firstalt">
			...
			<td><input type="text" name="con[name]" size="50" value="<?php echo $caiji_config['name']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ></td>
		</tr>

		<tr nowrap class="firstalt">
			<td width="260"><b>目标网站名称</b><br>
			<font color="#666666">多个用符号  * 分隔</font><br><font color="red">注：不要只填写字母或者域名，否则替换出错</font></td>
			<td><input type="text" name="con[from_title]" id="from_title" size="50" value="<?php echo $caiji_config['from_title']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ></td>
		</tr>

		<tr nowrap class="firstalt">
			<td width="260"><b>目标站地址</b><br>
			<font color="#666666">需要采集的目标网站地址</font><br><font color="red">注：http://或https://开头</font></td>
			<td><input type="text" name="con[from_url]" id="from_url" size="50" value="<?php echo $caiji_config['from_url']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" >&nbsp;<select name="con[charset]" >
		...
		<tr nowrap class="firstalt">
			...
			<td><input type="text" name="con[other_url]" id="other_url" size="50" value="<?php echo $caiji_config['other_url']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ></td>
		</tr>
		...
		<tr nowrap class="firstalt">
			...
			<td><input type="text" name="con[resdomain]" id="resdomain" size="50" value="<?php echo $caiji_config['resdomain']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ></td>
		</tr>
		...
		<tr nowrap class="firstalt">
			...
			<td><input type="text" name="con[img_delay_name]" size="50" value="<?php echo $caiji_config['img_delay_name']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" > <font color="red">一般不用设置</font></td>
		</tr>

		<tr nowrap class="firstalt">
			<td width="260"><b>目标站搜索地址</b><br>
			<font color="#666666">目标站搜索地址，有域名的要带上</font></td>
			<td><input type="text" name="con[search_url]" id="search_url" size="50" value="<?php echo $caiji_config['search_url']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" >&nbsp;<select name="con[search_charset]" >
		...
		<tr nowrap class="firstalt">
			...
			<td><textarea name="con[licence]" style="height: 80px; width: 550px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['licence']); ?></textarea></td>
		</tr>
		...
		<tr nowrap class="firstalt">
			...
			<td>开始标记 
				<textarea name="con[body_start]" style="height: 100px; width: 200px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['body_start']); ?></textarea>&nbsp;结束标记
				<textarea name="con[body_end]" style="height: 100px; width: 200px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['body_end']); ?></textarea>
			</td>
		</tr>
		<tr nowrap class="firstalt">
			...
			<td>
				<input name="siftags[]" type="checkbox" value="iframe" <?php if(in_array('iframe',$caiji_config['siftags'])||$ac=='add'){ ?>checked<?php } ?> /> iframe
		...
		<tr nowrap class="firstalt">
			...
			<td>
				<input name="siftags[]" type="checkbox" value="outa" <?php if(in_array('outa',$caiji_config['siftags'])){ ?>checked<?php } ?> /> <font color="red">站外</font>链接
...
<?php if($ac=='add'&&$caiji_config['replacerules']==''){$caiji_config['replacerules']='/----------------文字替换（本行格式为注释,仅用于方便查看,下同）----------------/
##########
这里可以写替换规则
##########
/----------------图片替换----------------/
...
这里可以写替换规则
##########';} ?>
			<td><textarea name="con[replacerules]" style="height: 450px; width: 750px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['replacerules']); ?></textarea></td>
		</tr>
		<tr nowrap class="firstalt">
			...
			<td><textarea name="con[siftrules]" style="height: 250px; width: 750px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['siftrules']); ?></textarea></td>
		</tr>
		<tr nowrap class="firstalt">
		...
		<tr class="firstalt replace_before_body"<?php if(!$caiji_config['replace_before_on'])echo " style='display:none'"; ?>>
			...
			<td><textarea name="con[replacerules_before]" style="height: 150px; width: 750px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['replacerules_before']); ?></textarea></td>
		</tr>
		<tr nowrap class="firstalt replace_before_body"<?php if(!$caiji_config['replace_before_on'])echo " style='display:none'"; ?>>
			...
			<td><textarea name="con[siftrules_before]" style="height: 150px; width: 750px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['siftrules_before']); ?></textarea></td>
		</tr>
		</tbody>
<script>
$(function() {
...
<?php if(empty($caiji_config['zdy'])){$caiji_config['zdy']=array(array('name'=>'','ename'=>'','body'=>'',),);}foreach($caiji_config['zdy']as $k=>$vo){ ?>
						<tr class="firstalt item<?php echo $k; ?>" itemid="<?php echo $k; ?>">
							...
							<div class="zdy_body_<?php echo $k; ?>"<?php if($vo['type'])echo ' style="display:none"'; ?>><textarea name="zdy[<?php echo $k; ?>][body]" style="height: 100px; width: 450px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($vo['body']); ?></textarea></div>

							<div class="zdy_regx_<?php echo $k; ?>"<?php if($vo['type']!=2)echo ' style="display:none"'; ?>><textarea name="zdy[<?php echo $k; ?>][regx]" style="height: 100px; width: 450px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($vo['regx']); ?></textarea></div>

							<div class="zdy_replace_<?php echo $k; ?>"<?php if($vo['type']!=1)echo ' style="display:none"'; ?>>
								开始标记 <textarea name="zdy[<?php echo $k; ?>][start]" style="height: 100px; width: 200px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($vo['start']); ?></textarea>
								&nbsp;结束标记
								<textarea name="zdy[<?php echo $k; ?>][end]" style="height: 100px; width: 200px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($vo['end']); ?></textarea>
							</div>
							</td>
...
$(document).ready(function(){
	...
	$("#form").submit(function(e){
		...
		<tr nowrap class="firstalt">
			...
			<td><textarea name="con[css]" style="height: 100px; width: 550px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['css']); ?></textarea></td>
		</tr>
		</tbody>
		...
		<tr nowrap class="firstalt">
			...
			<td><input type="text" name="con[tplfile]" id="tplfile" size="10" value="<?php echo $caiji_config['tplfile']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" > 留空则默认为index.html</td>
		</tr>
<?php $plusArr=array();if($caiji_config['plus']){$plusArr=explode(',',$caiji_config['plus']);}if(is_dir(VV_DATA.'/plus')){$arr=scandirs(VV_DATA.'/plus');unset($arr[0],$arr[1]);} ?>
<style type="text/css">
.custom-header{
  text-align: center;
  padding: 3px;
  ...
  color: #fff;
		...
		<tr nowrap class="firstalt">
			...
			<?php if($arr){define('VV_PLUS',true);$dir=VV_DATA.'/plus';foreach($arr as $k=>$vo){$plusfile=$dir.'/'.$vo.'/'.$vo.'.class.php';if(!is_dir($dir.'/'.$vo)||!is_file($plusfile)){continue;}require_once($plusfile);$plusclass=new $vo;$plusinfo=$plusclass->info; ?>
		...
		<tr nowrap class="firstalt">
			...
			<td><textarea name="con[cookie]" style="height: 100px; width: 550px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['cookie']); ?></textarea></td>
		</tr>
		<tr nowrap class="firstalt">
			<td width="260"><b>自定义浏览器标识（user-agent）</b><br>
			<font color="#666666">可伪造浏览器，伪造蜘蛛爬行</font></td>
			<td><input type="text" name="con[user_agent]" id="user_agent" style="width:300px;" value="<?php echo $caiji_config['user_agent']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" >&nbsp;<select onchange="$('#user_agent').val(this.value);">
				<option value="">自定义</option>
				<option value="Baiduspider/2.0+(+http://www.baidu.com/search/spider.htm)" <?php if($caiji_config['user_agent']=='Baiduspider/2.0+(+http://www.baidu.com/search/spider.htm)')echo " selected"; ?>>模拟百度蜘蛛</option>
				<option value="Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)" <?php if($caiji_config['user_agent']=='Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)')echo " selected"; ?>>模拟谷歌蜘蛛</option>
			</select></td>
		</tr>
		<tr nowrap class="firstalt">
			<td width="260"><b>自定义来路</b><br>
			<font color="#666666">伪造来路，不填写则自动伪造为目标站url</font></td>
			<td><input type="text" name="con[referer]" style="width:300px;" value="<?php echo $caiji_config['referer']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ></td>
		</tr>
		...
		<tr nowrap class="firstalt">
			...
			<br>
			<?php if(function_exists('curl_init')&&function_exists('curl_exec')){echo '<font color="green">你的空间支持curl，支持代理IP功能</font>';}else{echo '<font color="red">你的空间不支持curl，不能使用代理IP功能</font>';} ?>
			
			
			</font>
			</td>
			<td><input type="text" name="con[ip]" style="width:300px;" value="<?php echo $caiji_config['ip']; ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" >&nbsp;<select name="con[ip_type]">
