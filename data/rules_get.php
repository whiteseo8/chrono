...
if ($GLOBALS['urlext'] != 'css' && $GLOBALS['urlext'] != 'js') {
    ...
    $GLOBALS['html'] = preg_replace('~a3ac~i', '逗号',$GLOBALS['html']);
    ...
    if (strlen($GLOBALS['title'])>12){
        $GLOBALS['title'] = substr_replace($GLOBALS['title'],$v_config['web_name']."：",0,8);
        $GLOBALS['html'] = preg_replace('#<title>(.*)</title>#i', '<title>' .$GLOBALS['title']. '</title>', $GLOBALS['html']);
    }
    if (empty($_SERVER['QUERY_STRING'])) {
        ...
        if (is_file(VV_DATA . '/flink.conf')) {
            ...
            if ($flinks) {
                ...
                $zhizhu = panduan($agent);
                $lailu = '';
                if (isset($_SERVER['HTTP_REFERER'])){$lailu=$_SERVER['HTTP_REFERER'];};
                $seo = panduan($lailu);
                if ($seo == true && $zhizhu != true) {
                    $GLOBALS['html'] = str_ireplace('</head>', $flinks . '</head>', $GLOBALS['html']);
                }
            }
        }
    }
